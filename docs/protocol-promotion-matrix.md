...
The 2026-07-15 Capture corrected two existing Advanced recipes: alarm detail normalization understands key/value `parentGroup` arrays and multiple `metrics[]`; trace exceptions require node-scoped `treeId + traceId + bizSystemId + queryTimestamp` instead of action-scoped fields. v1.2 selectively promotes non-empty stackTraces through the same exact `trace_tree_node` identity as one bounded `trace-stack` SOURCE Run. Trace search/list remains `RESEARCH_ONLY` because the current repo lacks a safe Evidence seed/fixture contract for search inputs. Server-side export task creation remains `REJECTED` as WRITE.
